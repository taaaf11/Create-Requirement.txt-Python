            ...
            for i in splitted:
                if "." in i:
                    ...
                    try:
                        ...
                        continue
        elif "import" and "as" in splitted:
            imports.append(splitted[splitted.index("import")+1])
            for i in splitted:
                if "." in i:
                    if i.index(".") == 0:
                        imports.append(i[i.index(".")+1:])
                    ...
                    try:
                        ...
                        continue
        elif "from" and "import" and "as" in splitted:
            imports.append(splitted[splitted.index("from")+1])
            for i in splitted:
                if "." in i:
                    if i.index(".") == 0:
...
def create_file():
    print("Creating file... ")
    requirements = verify_imports(cmd[1])
    if len(requirements) != 0:
        ...
        print("No requirement required to add!")
    input("\nHit Enter to exit! ")
